...
impl HostState {
    ...
    fn take_host_frame_request(&mut self) -> Option<Vec<u8>> {
            ...
            .define_typed(
                ...
                |caller: polkavm::Caller<'_, HostState>,
                 ...
                 -> Result<u32> {
                    ...
                    caller.user_data.charge_hostcall(length)?;
                    if caller.user_data.host_frame_requests.len() == MAX_QUEUED_HOST_FRAMES
                        || caller
                            .user_data
                            .host_frame_request_bytes
                    ...
                    let bytes = read_guest_memory(caller.instance, pointer, length)?;
                    caller.user_data.host_frame_request_bytes += bytes.len();
                    caller.user_data.host_frame_requests.push_back(bytes);
                    Ok(0)
                },
            )
            .context("define host_frame_send")?;
            ...
            .define_typed(
                ...
                |caller: polkavm::Caller<'_, HostState>,
                 ...
                 -> Result<i32> {
                    caller.user_data.charge_hostcall(0)?;
                    let Some(required_len) =
                        caller.user_data.host_frame_responses.front().map(Vec::len)
                    else {
                        return Ok(0);
                    };
                    let required = i32::try_from(required_len)
                        .map_err(|_| anyhow!("host-frame response length overflow"))?;
                    if (capacity as usize) < required_len {
                        return Ok(-required);
                    }
                    caller.user_data.charge_hostcall_bytes(required_len)?;
                    let response = caller.user_data.host_frame_responses.front().unwrap();
                    caller
                        .instance
                        .write_memory(pointer, response)
                        .map_err(|error| anyhow!("write host-frame response: {error:?}"))?;
                    caller.user_data.host_frame_responses.pop_front();
                    caller.user_data.host_frame_response_bytes -= required_len;
                    Ok(required)
                },
            )
            .context("define host_frame_poll")?;
...
mod tests {
    ...
    #[test]
    fn host_frame_queues_enforce_frame_count_and_byte_limits() {
        let mut state = HostState::new(
            HashMap::new(),
            PresentationProfile::Framebuffer,
            false,
        ...
        assert!(state
            .queue_host_frame_response(vec![0; MAX_HOST_FRAME_BYTES + 1])
            .is_err());

        for _ in 0..MAX_QUEUED_HOST_FRAMES {
            state
                .queue_host_frame_response(vec![0; 1])
                .expect("bounded response should queue");
        }
        assert!(
            state.queue_host_frame_response(vec![0; 1]).is_err(),
            "frame-count overflow must fail closed"
        );

        state.host_frame_responses.clear();
        state.host_frame_response_bytes = 0;
        for _ in 0..(MAX_QUEUED_HOST_FRAME_BYTES / MAX_HOST_FRAME_BYTES) {
            state
                .queue_host_frame_response(vec![0; MAX_HOST_FRAME_BYTES])
        ...
        state.host_frame_requests.push_back(vec![1, 2, 3]);
        state.host_frame_request_bytes = 3;
        assert_eq!(state.take_host_frame_request(), Some(vec![1, 2, 3]));
        assert_eq!(state.host_frame_request_bytes, 0);
    }
    #[test]
